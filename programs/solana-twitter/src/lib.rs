...
declare_id!("C2yDMCNZ5BSYR3z8hCEHfRQAiKvD1ZDQsE4gHGQvPFWw");
...
// Account: This is an account type provided by Anchor. It wraps the AccountInfo in another struct that parses the data according to an account struct provided as a generic type.
// In the example above, Account<'info, Tweet> means this is an account of type Tweet and the data should be parsed accordingly.
// Signer: This is the same as the AccountInfo type except we're also saying this account should sign the instruction.

#[derive(Accounts)]
pub struct SendTweet<'info> {
    #[account(init, payer = author, space = Tweet::LEN)] //account constraints
